...
public class MedicalServiceImpl implements MedicalService {
    ...
    public void checkTemperature(String patientId, BigDecimal temperature) {
        PatientInfo patientInfo = getPatientInfo(patientId);
        if(temperature.compareTo(patientInfo.getHealthInfo().getNormalTemperature()) > 0
    || temperature.compareTo(patientInfo.getHealthInfo().getNormalTemperature().
                subtract(new BigDecimal("1.5"))) < 0){
//        if (patientInfo.getHealthInfo().
//                getNormalTemperature().subtract(new BigDecimal("1.5")).compareTo(temperature) > 0){
            String message = String.format("Warning, patient with id: %s, need help", patientInfo.getId());
            alertService.send(message);
        }
    }
